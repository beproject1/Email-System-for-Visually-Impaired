...
class CheckMailbox(ImapServer.ImapServerClass):
    ...
    def __init__(self):
        ...
        self.emails_in_the_form_of_lists_seen = []
        self.emails_in_the_form_of_lists_unseen = []
        
        self.normal_emails_all = []
        self.normal_emails_seen = []
        self.normal_emails_unseen = []
...
#---------------------------------------------------------------------------------------------------------------#    
    ...
    def checkMailboxForSeenEmails(self):

        if(self.normal_emails_seen == []):
            self.createListContainingSeenEmails()
    
        for email_message in self.normal_emails_seen:
            self.displayEmail(email_message)

    def createListContainingSeenEmails(self):
        self.selectAppropriateMailbox()
        self.fetchAllSeenEmailIdNumbers()
        self.generateRawEmailListForSeenEmails(self.emails_in_the_form_of_lists_seen)
        self.generateNormalEmailListForSeenEmails(self.emails_in_the_form_of_lists_seen, self.normal_emails_seen)

    def fetchAllSeenEmailIdNumbers(self):
        _, self.email_id_list = self.my_mail.search(None, "SEEN")

    def generateRawEmailListForSeenEmails(self, rawEmailList):
        for self.email_id_number in self.email_id_list[0].split():
            _, self.email_in_the_form_of_list = self.my_mail.fetch(self.email_id_number, "(RFC822)")
            rawEmailList.append(self.email_in_the_form_of_list)

    def generateNormalEmailListForSeenEmails(self, rawEmailList, normalEmailList):
        for raw_email in rawEmailList[::-1]:
            for individual_component in raw_email:
                if(type(individual_component) is tuple):
                   normal_email = email.message_from_bytes(individual_component[1])
...
#--------------------------------------------------------------------------------------------------------------------#
    ...
    def checkMailboxForUnseenEmails(self):

        if(self.normal_emails_unseen == []):
                self.createListContainingUnseenEmails()
        
        for email_message in self.normal_emails_unseen:
                self.displayEmail(email_message)

    def createListContainingUnseenEmails(self):
        self.selectAppropriateMailbox()
        self.fetchAllUnseenEmailIdNumbers()
        self.generateRawEmailListForUnseenEmails(self.emails_in_the_form_of_lists_unseen)
        self.generateNormalEmailListForUnseenEmails(self.emails_in_the_form_of_lists_unseen, self.normal_emails_unseen)

    def fetchAllUnseenEmailIdNumbers(self):
        _, self.email_id_list = self.my_mail.search(None, "UnSeen")

    def generateRawEmailListForUnseenEmails(self, rawEmailList):
        for self.email_id_number in self.email_id_list[0].split():
            _, self.email_in_the_form_of_list = self.my_mail.fetch(self.email_id_number, "(RFC822)")
            rawEmailList.append(self.email_in_the_form_of_list)

    def generateNormalEmailListForUnseenEmails(self, rawEmailList, normalEmailList):
        for raw_email in rawEmailList[::-1]:
            for individual_component in raw_email:
                if(type(individual_component) is tuple):
                   normal_email = email.message_from_bytes(individual_component[1])
...
if __name__ == "__main__":
    ...
    #check_mailbox.checkMailboxForParticularPhrase("laptop")
    #check_mailbox.displayUnseenEmails()
    check_mailbox.checkMailboxForUnseenEmails()
